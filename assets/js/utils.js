define({
	...
	if(videotype == 'youtube') {
		...
		if(this.getVideoType(url) == 'url') view.$('#videosrc').val(url);
        }
    },
	...
	else if (end != 0) url += '?end='+end;
	if(autoplay) {
		if((start != 0)||(end != 0)) url += '&autoplay=1';
		else url += '?autoplay=1';
	}
	return url;
    },
	...
	if(controls) url += '&hideControls=true'; 
	else url += '&hideControls=false';
        return url;
    },
