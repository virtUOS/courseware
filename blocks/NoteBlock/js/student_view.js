define(['assets/js/student_view', 'assets/js/url', 'assets/js/vendor/jspdf/jspdf.min'], function (StudentView, helper, jsPDF) {
    ...
    return StudentView.extend({
        ...
        postRender: function() {
            ...
            if ($notetype == "classic") $original.prop("class", $notetype+" add_toolbar");
        ...
        onDownload: function() {
            ...
            helper
                .callHandler(this.model.id, 'download', {content: $content})
                .then(
                    // success
                    function () {
        ...
        generateDocument: function() {
            ...
            if ($notes.length != 0) {
                ...
                $.each($notes, function($i){
                    var $content = doc.splitTextToSize($(this).val(), 60);
                    if (($i+1)%2 != 0) {
                        doc.setFillColor(254,250,188);
                        doc.rect(25, $top, 75, 75, 'F');
                        doc.text(30,$top+8, $content);
                    }
                    else {
                        doc.setFillColor(254,250,188);
                        doc.rect(110, $top, 75, 75, 'F');
                        doc.text(115,$top+8, $content);
                        $top += 85;
                    }
                    if (($i+1)%6 == 0) {
                        doc.addPage();
