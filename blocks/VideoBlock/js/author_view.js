...
export default AuthorView.extend({
  ...
  getUrl(view, videotype) {
    var url = '', message = '', youtubeid;
  ...
  setVideoData(view, url, videotype) {
    ...
    case 'youtube':
      ...
      if (this.getVideoType(url) == 'youtube') {
        ...
        view.$el.find('.videosrc').val(youtubeid);
        start = url.slice(url.indexOf('start=') + 6, url.length);
        start = start.split('&', 1);
        ...
        autoplay = url.slice(url.indexOf('autoplay=') + 9, url.length);
  ...
  buildMatterhornLink(url, startmin, startsec, autoplay) {
    var start = '';
    if (startmin != '') {
      start += startmin + 'm';
    }
