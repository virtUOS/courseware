...
class LinkBlock extends Block
{
    ...
    {
        return ["link_title" => $this->link_title];
    }
    ...
    private function getAttrArray()
    {
        return [
            ...
            "link_show_progress" =>
                $this->link_show_progress == "true" ? true : false,
        ];
    }
    ...
    private function getTargetId($target)
    {
        ...
        if (strpos($target, "sibling") > -1) {
            ...
            $id = $this->getModel()->parent->parent->parent->children[$num][
                "id"
            ];
        }
    ...
    private function getOtherSubchapters()
    {
        ...
        foreach ($allchapters as $key => $chapter) {
            ...
            foreach ($subchapters as $subchapter) {
                array_push($inotherchapters, [
                    "value" =>
                        "other_cpos" . $relativ_chapter_pos . "_item" . $i,
    ...
    public function save_handler(array $data)
    {
        ...
        if (isset($data["link_target"])) {
            $this->link_target = \STUDIP\Markup::purifyHtml(
                (string) $data["link_target"]
            );
        }
        if (isset($data["link_title"])) {
            $this->link_title = \STUDIP\Markup::purifyHtml(
                (string) $data["link_title"]
            );
        }
    ...
    public function pdfexport_view($context)
    {
        $pdf = $context["pdf"];
        $data = $this->student_view();
        if ($data["link_is_internal"] = $data["link_type"] === "internal") {
            $data["link_link_params"] = $pdf->serializeTCPDFtagParameters([
                $pdf->getLinkToItem($data["link_id"]),
                $data["link_title"],
            ]);
        }
